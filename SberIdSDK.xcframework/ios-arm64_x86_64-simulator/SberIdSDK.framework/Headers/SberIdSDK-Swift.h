...
enum SIDTrinity : NSInteger;
enum SIDAuthServiceType : NSInteger;

SWIFT_CLASS("_TtC9SberIdSDK14SIDAuthManager") SWIFT_AVAILABILITY(ios,unavailable,message="'SIDAuthManager' has been renamed to '_TtC9SberIdSDK10SIDManager'")
@interface SIDAuthManager : NSObject
+ (void)initSberIDWithClientId:(NSString * _Nonnull)clientId SWIFT_METHOD_FAMILY(none);
+ (void)authWithSberId:(SIDAuthRequest * _Nonnull)request viewController:(UIViewController * _Nullable)viewController;
+ (BOOL)soleLoginWebPageAuthWithSberIdRequest:(SIDAuthRequest * _Nonnull)request svcRedirectUrlString:(NSString * _Nonnull)svcRedirectUrlString viewController:(UIViewController * _Nonnull)viewController SWIFT_WARN_UNUSED_RESULT;
...
+ (void)logoutSession;
+ (void)sendOtherAuthWithServiceName:(NSString * _Nonnull)serviceName isSuccess:(enum SIDTrinity)isSuccess userStatus:(enum SIDTrinity)userStatus;
+ (void)sendOtherAuthWithService:(enum SIDAuthServiceType)service isSuccess:(enum SIDTrinity)isSuccess isUserNew:(enum SIDTrinity)isUserNew;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
SWIFT_CLASS("_TtC9SberIdSDK10SIDManager")
...
///
+ (void)getResponseFrom:(NSURL * _Nonnull)url completion:(SWIFT_NOESCAPE void (^ _Nonnull)(SIDAuthResponse * _Nonnull))completion;
/// Запускает сессию с предоставленным идентификатором сессии
/// \param clientId Идентификатор сессии
///
+ (void)startSession:(NSString * _Nonnull)sessionID;
...
/// Основное представление личного кабинета пользователя,
/// содержит <code>UITableView</code> для отображения ячеек.
SWIFT_CLASS("_TtC9SberIdSDK25SIDPersonalCabinetManager")
@interface SIDPersonalCabinetManager : NSObject <UITableViewDataSource>
/// Инициализатор представления.
/// Устанавливает интерактор и начальные настройки для <code>tableView</code>.
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
...
typedef SWIFT_ENUM(NSInteger, SIDTrinity, open) {
/// Позитивный
  SIDTrinityTrue = 0,
/// Негативный
  SIDTrinityFalse = 1,
